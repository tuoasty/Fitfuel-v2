...
export class ProfileController {
  ...
  async getProfile(@Req() req: Request, @Res() res: Response) {
    ...
    const profile = await this.profileService.profile({ userId: user.id });
    if (!profile) {
      return res
        .status(HttpStatus.NOT_FOUND)
        .send({ message: 'Please fill out your information first!' });
    } else {
  ...
  async completeProfile(
    ...
    if (file == null) {
      return res
        ...
        .send({ message: 'Please include a profile picture' });
    }
    const user: User = req['user'];
    ...
    let pictureUrl: string | null = await this.supabaseService.create(
      `profile-picture/${user.id}/${file.originalname}`,
      newFile,
    );
    if (pictureUrl == null) {
      pictureUrl = '';
